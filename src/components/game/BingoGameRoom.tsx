...
export default function BingoGameRoom({ gameId }: { gameId: string }) {
  ...
  return (
    <div className="space-y-4 max-w-lg mx-auto">
      ...
      {/* Real-time Number Drawer Display & 75-Ball Grid Board */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 text-center shadow-xl relative overflow-hidden space-y-3">
          ...
          {currentGame.currentBall ? (
            <div className="relative">
              ...
              >
                ...
                <span className="text-xl font-black leading-none">{currentGame.currentBall}</span>
              </div>
            </div>
          ) : (
        ...
        {/* 75-BALL GRID BOARD (10:20 Bingo Layout matching screenshot) */}
        ...
        <div className="pt-2 border-t border-slate-800/80 flex items-center justify-center gap-2">
          <button
            ...
            className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs px-3.5 py-1.5 rounded-xl transition flex items-center gap-1.5 shadow-md"
          >
          ...
          <button
            ...
            className={`text-xs font-bold px-3.5 py-1.5 rounded-xl transition flex items-center gap-1.5 border ${
              isAutoDrawing
      ...
      {/* BINGO CARD NUMBERS STATUS BAR (Exact screenshot implementation) */}
              ...
              activeGameCards.map((card) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-xl text-xs font-mono font-black transition flex items-center gap-1 shadow ${
                      isActive
          ...
          <div className="flex items-center gap-1.5">
            {activeGameCards.length > 0 ? (
              activeGameCards.map((c) => (
                <span
                  key={c.id}
                  className="px-3 py-0.5 rounded-lg bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-mono font-black text-xs flex items-center gap-1 shadow"
                >
              ...
              <span className="text-xs text-slate-500 italic">{t('noneYet')}</span>
            )}
          </div>
        </div>
      </div>
      ...
      {viewMode === 'ALL' && activeGameCards.length > 1 ? (
        ...
        <div className="glass-panel p-4 rounded-2xl border-amber-500/20 shadow-2xl space-y-3">
          <div className="flex items-center justify-between">
